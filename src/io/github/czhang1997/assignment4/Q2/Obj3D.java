...
class Obj3D {
    ...
    private float sunTheta = 0;
    private Vector<Point3D> w = new Vector<Point3D>();
    // World coordinates
                ...
                if (i == 0 || absi >= w.size() || w
                    ...
                    System.out.println(
                            "Invalid vertex number: " + absi +
                                    "mustbedefined,nonzeroandlessthan" + w.size());
                    return failing();
                }
...
// Sum of ambient, diffuse and specular illumination:
    ...
    void rotateLightVector(double radian) {

        double rho2 = getRho(xL, yL, zL);
        sunTheta += radian;
        double phi2 = getPhi(rho2, zL);

        xL = rho2 * Math.sin(phi2) * Math.cos(sunTheta);
        yL = rho2 * Math.sin(phi2) * Math.sin(sunTheta);
        zL = rho2 * Math.cos(phi2);
    }
    private double getRho(double x, double y, double z) {
        return Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2) + Math.pow(z, 2));
    }

    private double getPhi(double rho, double z) {
